...
osascript -e 'tell application "TDFileExplorer" to quit' >/dev/null 2>&1 || true
pkill -f "TDFileExplorer-server" 2>/dev/null || true
rm -rf /Applications/TDFileExplorer.app
ditto "$TMP/TDFileExplorer.app" /Applications/TDFileExplorer.app
# 다운로드 격리 해제 (서명 없는 사내 앱이라 Gatekeeper가 막는 것 방지)
xattr -dr com.apple.quarantine /Applications/TDFileExplorer.app 2>/dev/null || true
...
FINDER_EXT="/Applications/TDFileExplorer.app/Contents/PlugIns/TDFileExplorerFinder.appex"
...
open /Applications/TDFileExplorer.app
echo "설치 완료: TDFileExplorer v$VER — 이후 업데이트는 앱이 자동으로 안내합니다."
